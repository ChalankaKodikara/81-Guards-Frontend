...
function TimetablePopup({ timetableId, onClose }) {
  ...
  useEffect(() => {
    if (timetableId) {
      fetch(
        `${process.env.REACT_APP_API_ENDPOINT}/v1/hris/timetable/gettimetablebyid?timetableID=${timetableId}`
      )
  ...
  const handleSubmit = (e) => {
    ...
    fetch(
      `${process.env.REACT_APP_API_ENDPOINT}/v1/hris/timetable/updatetimetable?timetableID=${timetableId}`,
      {
        method: "PUT",
        headers: {
