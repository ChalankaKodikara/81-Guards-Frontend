  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/v1/hris/user/addUserRole`,
        {
          method: "POST",
          headers: {
    ...
    <div className="mx-10 mt-5">
      ...
      <div className="flex justify-between items-center mt-6">
        <div>
          ...
          <p className="text-lg font-semibold text-purple-700">Permission</p>
        </div>
        <div className="flex gap-6 items-center">
          <div className="text-xl font-bold text-purple-800">{currentDate}</div>
          <div className="text-xl font-bold text-purple-800">{currentTime}</div>
        </div>
      </div>
